...
function NewAdherent() {
  ...
  const handleSubmit = () => {
    if (type === "") {
      alert(
        "Le champ type est obligatoire ! Si jamais vous ne parvenez pas à choisir un type c'est soit parce vous n'avez pas de type ou soit parce que vos types sont à l'état fermé"
      );
    } else if (nature === "" || nom === "" || prenom === "") {
      alert("Veuillez remplir les champs ,nature, nom, prenom");
    } else {
      axios
        .post("http://localhost:8081/createadherent", {
          type,
          nature,
          societe,
          civilite,
        ...
        .then((res) => navigate(`/adherents/card/${res.data.insertId}/index`))
        .catch((err) => console.log(err));
    }
  };
  return (
